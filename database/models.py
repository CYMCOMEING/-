...
#     线程函数储存在字典中，使用type字段获得对应的函数
#     函数参数需要实现字符串化
#     """
#     __tablename__ = 'task'

#     id = Column(Integer, primary_key=True)
#     type = Column(Integer, nullable=True) # 0主页 1页面 2图片
#     comicid = Column(Integer, nullable=True)  # 将*args和**kwargs转成字符串，后面用eval()调用
#     url = Column(String, default='')  # 下载url
#     file_path = Column(String, default='')  # 文件路径
#     static = Column(Integer, default=0)  # 0等待中 1下载中 2完成(删除)

#     def __repr__(self):
#         return f'<Task({self.type}, {self.comicid}, {self.url}, {self.file_path}, {self.static})>'
